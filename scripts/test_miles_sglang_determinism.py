...
用法：Miles 用 --debug-rollout-only 启动后，在另一终端运行此脚本。
  ...
  python scripts/test_miles_sglang_determinism.py --host 172.17.0.2 --port 30000 --test-mode single --temperature 0
  python scripts/test_miles_sglang_determinism.py --host 172.17.0.2 --port 30000 --prompt "1+1等于几"
...
def run_single_mode(host, port, n_trials, prompt, temperature=0.0, quiet=False):
    """
    Single 模式：同一 prompt 发送 n_trials 次，每次单条请求。
    确定性时，所有输出应完全相同，Unique=1。
    """
    if not quiet:
        print(f"\n=== Single 模式 ({n_trials} 次单条请求, temperature={temperature}) ===")
    random.seed(42)
    texts = []
    ...
    n_unique = len(unique)
    print(f"Total: {len(texts)}, Unique: {n_unique}")
    if n_unique == 1:
        print("  -> 通过（所有输出一致）")
    else:
...
def run_prefix_mode(host, port, n_trials, n_start=1, temperature=0.0, quiet=False):
    """
...
def main():
    ...
    p.add_argument("--prompt", default="给我介绍下sglang", help="Single 模式下使用的 prompt")
    p.add_argument("--n-trials", type=int, default=15)
    p.add_argument("--n-start", type=int, default=1)
    p.add_argument("--temperature", type=float, default=0.5)
    p.add_argument("--quiet", "-q", action="store_true", help="仅输出摘要，减少每 trial 的 print")
    args = p.parse_args()

    if args.test_mode == "single":
        n_unique = run_single_mode(
            args.host, args.port, args.n_trials,
            prompt=args.prompt, temperature=args.temperature, quiet=args.quiet
        )
    else:
        n_unique = run_prefix_mode(
            args.host, args.port, args.n_trials, args.n_start,
            temperature=args.temperature, quiet=args.quiet
        )

    if n_unique is None:
        sys.exit(1)
    passed = (n_unique == 1) if isinstance(n_unique, int) else all(r == 1 for r in n_unique)
